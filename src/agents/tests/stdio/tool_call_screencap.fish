...
set request_call_screencap '{ "jsonrpc": "2.0", "id": 2, "method":"tools/call","params":{"name":"screencap", "arguments": {"description": "test screenshot"}}}'

begin
    echo $request_init
    sleep 1
    echo $notify_initialized
    echo $request_call_screencap
    sleep 3  # wait for screencapture to complete
end | uv run \
